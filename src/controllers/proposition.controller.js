...
//Add comments because of bullky file
...
exports.createProposition = async (req, res, next) => {
  ...
  try {
    ...
    const selectedFreelance = await Freelance.findById(
      req.body.freelance
    ).populate("user");
    ...
    sendEmail(
      selectedFreelance.user.email,
      "Proposition de mission",
      "Une entreprise vous a proposé une mission",
      "Vous pouvez accepter ou refuser la mission en cliquant sur ce lien"
    );
  ...
  try {
    ...
    switch (status) {
      ...
      case "REFUSED":
        ...
        sendEmail(
          me.email,
          "Refus de proposition de mission",
          "Un freelance a refusé la mission",
          `le freelance ${me.id} a refusé la mission`
        );
      ...
      case "ACCEPTED":
        ...
        sendEmail(
          ...
          "Un freelance a validé la mission",
          `le freelance ${me.id} a validé la mission`
        );
        //return success message
        return res.send({
          success: true,
