...
//Add comments because of bulky file
...
exports.updateMyCompany = async (req, res, next) => {
  try {
    ...
    const CompanyToModify = await Company.findByIdAndUpdate(
      me.company.id,
      req.body,
...
exports.forgotPassword = async (req, res, next) => {
  try {
    ...
    sendEmail(
      ...
      "password reset",
      "",
      "Veuillez cliquer sur ce lien pour regénérer un nouveau mot de passe"
    );
...
exports.updateUser = async (req, res, next) => {
  try {
    ...
    // find user and update
    const userToModify = await User.findByIdAndUpdate(req.params.id, req.body, {
      new: true,
    });
